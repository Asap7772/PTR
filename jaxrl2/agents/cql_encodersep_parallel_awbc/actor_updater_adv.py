...
def log_prob_update(
    ...
    def loss_fn(actor_params: Params) -> Tuple[jnp.ndarray, Tuple[Any, Dict[str, float]]]:
        ...
        things_to_log={
            'awbc_advantages_mean': advantage.mean(), 
            'awbc_advantages_std': advantage.std(),
            'awbc_advantages_max': advantage.max(), 
            'awbc_advantages_min': advantage.min(),
            'awbc_clipped_advantages_mean': clipped_advantage.mean(), 
            'awbc_clipped_advantages_std': clipped_advantage.std(),
            'awbc_clipped_advantages_max': clipped_advantage.max(), 
            'awbc_clipped_advantages_min': clipped_advantage.min(),
            'awbc_log_probs': log_prob_loss,
            'awbc_MSE': mse_loss, 
            'awbc_dataset_actions': batch['actions'],
            'awbc_pred_actions_mean': dist.distribution.loc, 
            'awbc_action_std': dist.distribution._scale_diag,
            'awbc_loss': actor_loss,
            'awbc_temperature': temperature,
        }
