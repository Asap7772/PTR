...
from jaxrl2.agents.drq.encoders import D4PGEncoder, ResNetV2Encoder
from jaxrl2.networks.encoders.networks import PixelMultiplexer
...
def _share_encoder(source, target):
    ...
    if 'encoder' in source.params:
        new_params = target.params.copy(
            add_or_replace={'encoder': source.params['encoder']})
    else:
        encoder_names = [k for k in source.params.keys() if k.startswith('encoder')]
        new_params = target.params
        for name in encoder_names:
            new_params = new_params.copy(
                add_or_replace={name: source.params[name]})
...
class DrQLearner(Agent):
    ...
    def __init__(self,
        ...
        if encoder == 'd4pg':
            encoder_def = D4PGEncoder(cnn_features, cnn_strides, cnn_padding)
        elif encoder == 'resnet':
