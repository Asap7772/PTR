...
from jaxrl2.agents.bc.bc_learner import _log_prob_update_jit
...
def _update_jit(
    ...
    _, new_actor, actor_info = _log_prob_update_jit(key, actor, batch)
...
class PixelBCLearner(Agent):
    ...
    def __init__(self,
        ...
        if encoder_type == 'small':
            ...
            encoder_def = ResNet18(norm=encoder_norm, use_spatial_softmax=use_spatial_softmax, softmax_temperature=softmax_temperature, use_multiplicative_cond=use_multiplicative_cond,
            use_spatial_learned_embeddings=use_spatial_learned_embeddings,)
        elif encoder_type == 'resnet_34_v1':
            print('using resnet34v1')
            encoder_def = ResNet34(norm=encoder_norm, use_spatial_softmax=use_spatial_softmax, softmax_temperature=softmax_temperature,use_multiplicative_cond=use_multiplicative_cond,
            use_spatial_learned_embeddings=use_spatial_learned_embeddings,)
    ...
    def update(self, batch: FrozenDict) -> Dict[str, float]:
        new_rng, new_actor, info = _update_jit(self._rng, self._actor, batch, self.color_jitter)
